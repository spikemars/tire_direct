...
import { createOrder, createCheckout, centsToDisplay, type OrderResponse } from "../lib/api"
...
function mapInstallLocation(region: string): string {
  if (region === "九龍") return "九龍灣旗艦店"
  if (region === "新界") return "沙田形象店"
  if (region === "港島") return "中環形象店"
  return ""
}
...
export default function OrderFlow({ product, payMode, onClose, onComplete }: OrderFlowProps) {
  ...
  const unitPrice = useMemo(() => parseUnitPrice(product.price), [product.price])
  ...
  async function handleStep1Next() {
    ...
    if (!product.sku) {
      setSubmitError("缺少產品 SKU，請返回重新選擇輪胎")
      return
    }
    ...
    try {
      const resp = await createOrder({
        ...
        install_location: mapInstallLocation(installRegion),
      })
          ...
          {step === 1 && (
            <section className="flex flex-col gap-4">
              <div className="border border-slate-700 bg-slate-900 px-4 py-3">
                ...
                <div className="mt-1 text-[10px] text-slate-500">SKU：{product.sku}</div>
                ...
                {orderResponse && (
                  <div className="mt-2 border-t border-slate-700 pt-2">
                    <div className="text-[10px] text-emerald-400">
                      ...
                      {payMode === "deposit"
                        ? ` · 定金 ${centsToDisplay(orderResponse.order.deposit_amount)}`
                        : ""}
                    </div>
                  </div>
                )}
                  ...
                  {[
                    {
                      ...
                      s: setServicePromise,
                      title: "服務商承諾",
                      body: "「呔直達承諾：您的訂金將托管於獨立帳戶，若服務商未能於 48 小時內確認備貨，訂金全額退回，無需任何理由。」",
                    },
                    {
                      v: clientPromise,
                      s: setClientPromise,
                      title: "客戶承諾",
                      body: "「訂單確認後，如客戶單方面取消，訂金（總額 30%）將作為服務商備貨成本補償，不予退還。此條款已符合香港消委會消費者合約指引。」",
                    },
                    {
                      v: qualityPromise,
                      s: setQualityPromise,
                      title: "品質保障",
                      body: "「安裝完成前，您將收到 6 位驗收碼。尾款僅在您輸入驗收碼確認驗收後方可放行。若輪胎規格與訂單不符，訂金全退並補償 HK$200。」",
                    },
                    {
                      v: allAgree,
                      s: setAllAgree,
                    ...
                    <label
                      key={title}
                      className="flex cursor-pointer items-start gap-2 border border-slate-800 bg-slate-950 px-3 py-2"
                    >
                      <input
                        type="checkbox"
                    ...
                    {orderResponse ? (
                      <>
                        ...
                        <div className="mt-2 border-t border-slate-800 pt-2 text-[10px] text-slate-400">
                          訂單號：
                          <span className="font-mono text-slate-200">{orderResponse.order.order_no}</span>
                        </div>
                        <div className="text-[10px] text-slate-400">
                          安裝日期：<span className="font-mono text-slate-200">{installDate || earliestDate}</span>
                        </div>
                        <div className="text-[10px] text-slate-400">
                          安裝地區：<span className="font-mono text-slate-200">{installRegion}</span>
                        </div>
                        <div className="mt-2 font-mono text-[11px] text-emerald-400">
                          {centsToDisplay(mainPayAmount)}（
                          {payMode === "deposit"
                            ? `定金 30% · 尾款 ${centsToDisplay(remainingAmount)}`
                            : "全額"}
                          ）
                        </div>
                        {previewRecord && (
                          <div className="mt-2 text-[10px] text-slate-500">
                            遮罩電話：{previewRecord.phone}
                          </div>
                        )}
                      </>
                    ) : (
                  ...
                  <div className="border border-slate-800 bg-slate-950 px-3 py-3">
                    ...
                    <div className="flex flex-col gap-2">
                      {[
                        {
                          key: "card" as const,
                          label: "信用卡（Stripe / Mock）",
                          desc: "安全快捷，支援 Visa / Mastercard",
                        },
                        {
                          key: "fps" as const,
                          label: "FPS 轉數快（稍後開通）",
                          desc: "QR Code 掃碼，支援所有香港銀行",
                        },
                        {
                          key: "payme" as const,
                          label: "PayMe（稍後開通）",
                          desc: "用 PayMe App 掃碼付款",
                        },
                      ].map((m) => (
                        <button
                          key={m.key}
                          ...
                          className={`flex flex-col border px-3 py-2 text-left text-[11px] transition-colors ${
                            paymentMethod === m.key
                              ...
                              : m.key !== "card"
                                ? "cursor-not-allowed border-slate-700 bg-slate-900 opacity-40"
        ...
        <footer className="border-t border-slate-800 bg-slate-950 px-6 py-3">
          <div className="flex items-center justify-between gap-3">
            <button
              type="button"
              onClick={() =>
                step === 1 ? requestClose() : setStep((p) => (p === 1 ? 1 : ((p - 1) as 1 | 2 | 3)))
              }
              ...
              <button
                ...
                className={`border px-5 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] ${
                  allTermsChecked
                    ...
                    : "cursor-not-allowed border-slate-700 bg-slate-900 text-slate-500 opacity-30"
                }`}
              >
                確認條款，前往付款 →
              </button>
            ...
            {step === 3 && (
              <button
                ...
                disabled={isSubmitting || !orderResponse}
                className="border border-blue-600 bg-blue-600 px-5 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-white hover:bg-blue-500 disabled:opacity-40"
              >
